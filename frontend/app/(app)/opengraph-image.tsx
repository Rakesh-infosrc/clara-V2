...
export default async function Image() {
  const pageTitle = cleanPageTitle(APP_CONFIG_DEFAULTS.pageTitle);
  const logoUri = APP_CONFIG_DEFAULTS.logoDark || APP_CONFIG_DEFAULTS.logo;
  ...
  // bg (optional)
  let bgSrcBase64: string | undefined;
  try {
    const bg = await getImageData('public/opengraph-image-bg.png');
    bgSrcBase64 = bg.base64;
  } catch {}

  // wordmark (optional)
  let wordmarkSrcBase64: string | undefined;
  let wordmarkSize: { width: number; height: number } | undefined;
  try {
    const wm = isLogoUriLocal ? await getImageData(wordmarkUri) : await getImageData(logoUri);
    wordmarkSrcBase64 = wm.base64;
    wordmarkSize = scaleImageSize(wm.dimensions, isLogoUriLocal ? 32 : 64);
  } catch {}

  // logo (optional)
  let logoSrcBase64: string | undefined;
  let logoSize: { width: number; height: number } | undefined;
  try {
    const lg = await getImageData(logoUri, 'public/lk-logo-dark.svg');
    logoSrcBase64 = lg.base64;
    logoSize = scaleImageSize(lg.dimensions, 24);
  } catch {}
    ...
    (
      ...
      <div
        style={{
          ...
          width: size.width,
          height: size.height,
          backgroundImage: bgSrcBase64 ? `url(${bgSrcBase64})` : undefined,
      ...
      >
        ...
        >
          {wordmarkSrcBase64 && wordmarkSize && (
