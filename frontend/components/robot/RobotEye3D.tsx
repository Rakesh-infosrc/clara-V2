...
function Eye({
  ...
  useFrame((state, delta) => {
    ...
    const voicePulse = emotion === 'happy' ? MathUtils.clamp(energy, 0, 1) * 1.2 : 0;
    ...
    if (iris.current) {
      ...
      if (mat) {
        ...
        mat.emissiveIntensity = Math.max(
          0,
          (c.glow + thinkPulse + voicePulse) * MathUtils.clamp(dim, 0, 1)
        );
        mat.clearcoat = 1;
    ...
    if (eyeWhite.current) {
      ...
      if (mat) {
        mat.emissiveIntensity =
          (0.4 + (emotion === 'happy' ? 0.2 : 0)) * MathUtils.clamp(dim, 0, 1);
      }
    }

    // Pupil breathing and jitter (never disappears)
    if (pupil.current) {
      const base = 0.22;
      let s = Math.max(0.07, base * c.pupil * (1 - 0.12 * MathUtils.clamp(energy, 0, 1)));
      // breathing effect
    ...
    // Natural blink: eyelids always open after blink
    ...
    if (dt >= 0 && dt <= BLINK_DUR) {
      blinkAmt = 0.5 - 0.5 * Math.cos((dt / BLINK_DUR) * Math.PI); // smooth open-close-open
    } else {
      blinkAmt = 0; // always open when not blinking
    }

    // Eyelids always restore to open
    if (topLid.current && bottomLid.current) {
      const open = MathUtils.clamp(c.eyelid, 0, 1);
      const blinkClose = 0.8 * blinkAmt;
    ...
    <group>
      ...
      {positions.map((x, i) => (
        <group key={i} position={[x, 0, 0]} scale={[0.9, 0.9, 0.9]}>
          <Eye emotion={emotion} energy={energy} dim={dim} />
        </group>
      ))}
    </group>
  );
...
export default function EyeScene({
  ...
  return (
    <Canvas camera={{ position: [0, 0, 2.4], fov: 35 }} dpr={[1, 2]}>
      ...
      <Face emotion={emotion as EmotionKey} eyes={eyes} energy={energy} dim={dim} />
    </Canvas>
  );
}
