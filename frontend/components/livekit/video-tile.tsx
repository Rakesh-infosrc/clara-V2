...
import { BACKEND_BASE_URL, cn } from '@/lib/utils';
...
export const VideoTile: React.FC<VideoTileProps> = ({ trackRef, className, ref }) => {
  ...
  const verifyFace = useCallback(async () => {
    ...
    const track = publication?.track as { attachedElements?: Element[] } | undefined;
    const attached = track?.attachedElements ?? [];
    const videoEl = attached[0] as HTMLVideoElement | undefined;
    ...
    try {
      ...
      canvas.toBlob(
        async (blob) => {
          ...
          try {
            const res = await fetch(`${BACKEND_BASE_URL}/face_login`, {
              method: 'POST',
              body: formData,
            });
  ...
  }, [maxScans, trackRef, verification.status]);
  ...
  }, [trackRef, verification.status, verifyFace]);
  ...
  return (
    ...
    >
      ...
      {verification.message && (
        <div
          className={cn(
            'absolute top-2 right-2 left-2 rounded px-2 py-1 text-xs font-medium text-white',
      ...
      {verification.status === 'verified' && verification.employeeName && (
        <div className="absolute right-2 bottom-2 left-2 rounded bg-green-600 px-2 py-1 text-center text-xs font-medium text-white">
